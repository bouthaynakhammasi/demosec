...
public class DatabaseFixer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        // ── 6. Fix dtype discriminator values (CRITICAL LOGIN FIX) ──────────
        // Old data may have mixed-case dtype values (e.g. 'User', 'Patient')
        // but @DiscriminatorValue uses ALL_CAPS. This causes:
        // "Unrecognized discriminator value: User" on login.
        try {
            jdbcTemplate.execute("UPDATE users SET dtype = 'USER'       WHERE dtype = 'User'");
            jdbcTemplate.execute("UPDATE users SET dtype = 'PATIENT'    WHERE dtype = 'Patient'");
            jdbcTemplate.execute("UPDATE users SET dtype = 'PHARMACIST' WHERE dtype = 'Pharmacist'");
            jdbcTemplate.execute("UPDATE users SET dtype = 'DOCTOR'     WHERE dtype = 'Doctor'");
            jdbcTemplate.execute("UPDATE users SET dtype = 'ADMIN'      WHERE dtype = 'Admin'");
            jdbcTemplate.execute("UPDATE users SET dtype = 'CLINIC'     WHERE dtype = 'Clinic'");
            jdbcTemplate.execute("UPDATE users SET dtype = 'NUTRITIONIST'      WHERE dtype = 'Nutritionist'");
            jdbcTemplate.execute("UPDATE users SET dtype = 'LABORATORY_STAFF'  WHERE dtype = 'LaboratoryStaff'");
            jdbcTemplate.execute("UPDATE users SET dtype = 'HOME_CARE_PROVIDER' WHERE dtype = 'HomeCareProvider'");
            System.out.println("✅ dtype discriminator values normalized to ALL_CAPS.");
        } catch (Exception e) {
            System.out.println("[DatabaseFixer] dtype fix skipped: " + e.getMessage());
        }

        // ── 7. Fix medical_events schema leftovers ──────────────────────────
        // Removes stale NOT NULL constraints on medical_events after transition to InheritanceType.JOINED
        try {
            jdbcTemplate.execute("ALTER TABLE medical_events MODIFY COLUMN capacity INT NULL");
            jdbcTemplate.execute("ALTER TABLE medical_events MODIFY COLUMN venue_name VARCHAR(255) NULL");
            jdbcTemplate.execute("ALTER TABLE medical_events MODIFY COLUMN address VARCHAR(255) NULL");
            jdbcTemplate.execute("ALTER TABLE medical_events MODIFY COLUMN city VARCHAR(255) NULL");
            jdbcTemplate.execute("ALTER TABLE medical_events MODIFY COLUMN country VARCHAR(255) NULL");
            jdbcTemplate.execute("ALTER TABLE medical_events MODIFY COLUMN platform_name VARCHAR(255) NULL");
            jdbcTemplate.execute("ALTER TABLE medical_events MODIFY COLUMN meeting_link VARCHAR(255) NULL");
            System.out.println("✅ medical_events stale constraints removed.");
        } catch (Exception e) {
            System.out.println("[DatabaseFixer] medical_events fix skipped: " + e.getMessage());
        }
