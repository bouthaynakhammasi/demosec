...
public class LabRequestcontroller {
    ...
    @PostMapping
    public ResponseEntity<LabRequestResponse> create(@Valid @RequestBody LabRequestRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(labRequestService.create(request));
    }
    ...
    public ResponseEntity<List<LabRequestResponse>> getByPatient(@PathVariable Long patientId) {
        return ResponseEntity.ok(labRequestService.getByPatient(patientId));
    }

    @GetMapping("/doctor/{doctorId}")
    public ResponseEntity<List<LabRequestResponse>> getByDoctor(@PathVariable Long doctorId) {
        return ResponseEntity.ok(labRequestService.getByDoctor(doctorId));
    }

    @GetMapping("/laboratory/{laboratoryId}")
    public ResponseEntity<List<LabRequestResponse>> getByLaboratory(@PathVariable Long laboratoryId) {
        return ResponseEntity.ok(labRequestService.getByLaboratory(laboratoryId));
    }

    @PatchMapping("/{id}/status")
    public ResponseEntity<LabRequestResponse> updateStatus(@PathVariable Long id,
