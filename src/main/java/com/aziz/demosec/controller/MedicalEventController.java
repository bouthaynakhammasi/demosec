...
public class MedicalEventController {
    ...
    public ResponseEntity<MedicalEventResponse> create(
            @ModelAttribute MedicalEventCreateRequest request,
            @RequestParam(value = "image", required = false) MultipartFile image) {
        if (image != null && !image.isEmpty()) {
            String imageUrl = medicalEventService.storeImage(image);
            request.setImageUrl(imageUrl);
        }
        return ResponseEntity.ok(medicalEventService.create(request));
    }
    ...
    public ResponseEntity<MedicalEventResponse> update(
            ...
            @ModelAttribute MedicalEventUpdateRequest request,
            @RequestParam(value = "image", required = false) MultipartFile image) {
        if (image != null && !image.isEmpty()) {
            String imageUrl = medicalEventService.storeImage(image);
            request.setImageUrl(imageUrl);
        }
        return ResponseEntity.ok(medicalEventService.update(id, request));
    }

    @GetMapping("/public/{id}")
    public ResponseEntity<MedicalEventResponse> getEventById(@PathVariable Long id) {
        return ResponseEntity.ok(medicalEventService.getById(id));
    }
    ...
    // TODO: Implement participation methods in IMedicalEventService
    /*
